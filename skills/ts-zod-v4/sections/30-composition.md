...
## What it is
Schema composition beyond leaf values: unions, discriminated unions, intersections, optionality wrappers, recursive
shapes, and custom refinements.
...
- Use `z.xor()` when exactly one option must match, including rejecting values that match several options.
- Use `z.getDiscriminatedOption()` to retrieve a declared discriminated-union member as its original object schema.
...
const SuccessSchema = z.getDiscriminatedOption(Result, "ok")
...
const ExactlyOneIdentifier = z.xor([
...
- Using `z.xor()` with overlapping object schemas and accidentally matching several options
