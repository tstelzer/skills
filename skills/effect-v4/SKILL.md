---
name: effect-v4
description: Canonical handbook for Effect v4 in TypeScript. Use for Effect code on effect@4 / beta — imports from effect/unstable/*, Context.Service, Effect.fn, Schema.TaggedErrorClass, single-versioned @effect/* packages. For effect@3, use effect-v3 instead.
---
...
If it's `effect@3` / `@effect/platform@0.x` style, use `effect-v3`.
...
- **`examples/02_stream/`** — Streams: effectful, pull-based sequences.
  - `10_creating-streams.ts` — streams from iterables, effects, pagination, async iterables, events, callbacks, Node readables.
...
Migrating v3 → v4. Start at **`migration/MIGRATION.md`** (overview: versioning, package consolidation, the `effect/unstable/*` system). It indexes:
